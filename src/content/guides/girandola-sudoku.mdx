...
Girandola sudoku trains the one habit that transfers everywhere: scanning a spread-out unit for used digits. Watch placements land one at a time in the [hints tool](/), warm up on a conventional board in the [step-by-step sudoku guide](/guides/sudoku-step-by-step), and generate unlimited fresh 9x9 puzzles at your level in the [maker](/maker). If the pinwheel felt thin, the same series shows how far a constraint can stretch in the other direction — the [anti-knight sudoku guide](/guides/anti-knight-sudoku) cuts clues by 11 with a rule that touches every cell, and the [sandwich sudoku guide](/guides/sandwich-sudoku) cuts 12.5 with row sums. Bigger boards, same window habit: [16x16 mega sudoku](/mega) runs the identical row, column, and box logic with sixteen digits.