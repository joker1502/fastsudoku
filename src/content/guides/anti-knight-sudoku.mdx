...
Anti-knight sudoku hones the candidate-hunting habit you already use here, just one step earlier: you kill digits by position before you ever write them. Watch single candidates fall one at a time in the [hints tool](/), warm up on a conventional board step by step in the [step-by-step sudoku guide](/guides/sudoku-step-by-step), and generate unlimited fresh puzzles at your level in the [maker](/maker). For a bigger canvas for the same habit, [16x16 mega sudoku](/mega) runs the identical row, column, and box logic with sixteen digits — where the knight rule multiplies those bans across four times the board.